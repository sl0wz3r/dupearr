...
| Trusted proxies (advanced) | *(empty)* | `DUPEARR__AUTH__TRUSTEDPROXIES`. Only behind a reverse proxy (SWAG, Nginx Proxy Manager): the proxy container's IP on your custom Docker network (give it a fixed IP; not the gateway). Dupearr then believes the client address the proxy reports; required for *External* authentication. A value here overrides *Trusted Proxies* in *Settings → General* (read-only there); leave it empty to set it there instead. |
| Allowed hosts (advanced) | *(empty)* | `DUPEARR__AUTH__ALLOWEDHOSTS`. Optional: the host name(s) Dupearr is reached by through the proxy. Overrides *Allowed Hosts* in *Settings → General*; leave it empty to set it there instead. |
