...
| Several Plex servers | Each server is scanned and grouped on its own; the same title on two servers is **not** a duplicate. Removals do **not** yet check the other servers: server A can remove a file that is the only copy server B lists (when B indexes fewer folders), and a remote server without path mappings whose paths equal local ones can have its copy matched to the local Radarr/Sonarr file (approving that group would remove the local file). Until this is fixed, map every server's folders, keep the \*arr and Dupearr recycle bins on, and review groups of other servers carefully ([research](docs/research/multi-server.md#27-what-happens-today-four-scenarios)). | Cross-server protection first, then cross-server groups ("keep one copy across all my servers"). |
